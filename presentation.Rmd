...
### We investigated.   `violentPerPop` variable
...
.panel[.panel-name[Violent per 100k]
...
tryCatch({
  ...
  if (!exists("data")) {
    stop("The 'data' data frame is not loaded. Please load it before running this code.")
  }
  ...
  ggplot(data = my_sf_merged) +
    ...
    labs(
      ...
      subtitle = "Data column: violentPerPop",
...
.panel[.panel-name[Police per 100km] 
.pull-left[
```{r echo=FALSE, out.width="100%"}
...
tryCatch({
  ...
  my_sf_merged <- my_sf %>%
    left_join(data, by = c("name" = "state_name")) %>%
    mutate(value = ifelse(is.na(violentPerPop), 0.01, policePerPop))
  ...
  ggplot(data = my_sf_merged) +
    ...
    scale_fill_gradientn(
      colours = palette,
      limits = c(min(my_sf_merged$value, na.rm = TRUE), 500),
    ...
    labs(
      title = "Police per 100,000 Population by State",
      subtitle = "Data column: policePerPop",
...
tryCatch({
  ...
  ggplot(data = my_sf_merged) +
    ...
    scale_fill_gradientn(
      colours = palette,
      limits = c(min(my_sf_merged$value, na.rm = TRUE), 20),
      name = "Percentage of population between 16 and 24",
      na.value = "grey50"
    ) +
    theme_minimal() +
    theme(
...
ggplot(data, aes(
  ...
  geom_smooth(method = "lm", color = "black", se = FALSE) +
  labs(
    title = "Violent Crimes per Population vs Percentage of Population Aged 16-24",
    x = "Percentage of Population Aged 16-24",
    y = "Violent Crimes per Population"
  ) +
  theme_bw() +
  theme(
...
---
#for previous slide with the maps

#one map will be just violent crimes per pop
#one will be violent per pop and police per pop next ro each other
#one will be percentage of people between 16-24
