...
- test on 1-9 fold and find results on 10 fold
...
- make categorical varialbes; high, med, low
- mutate number of state to name
- split by state
- remove 0 and 1 
- transfer from 
...
data <- read.csv("data/crime/communities.data", header = FALSE, na.string = c("?", 0.00, 1.00))
...
data <- data %>%
  ...
  select(state_name,communityname,everything(),-state)
...
# view column names, add description
...
race <- data %>%
  select("state_name", "communityname",  "population","racepctblack","racePctWhite","racePctAsian","racePctHisp") 
...
age <- data %>%
  select("state_name", "communityname", "population", "agePct12t21", "agePct12t29", "agePct16t24", "agePct65up")
...
plot_y <- function (data, variable) {
  plot <- ggplot(data = data,
    mapping = aes_string(x="ViolentCrimesPerPop", y = variable)) +
  geom_point() + 
  xlim(0.0,1.0) + 
  ylim(0.0,1.0) +
