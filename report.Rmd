...
>>>>>>> 0a5076cf9d6765546c35c272d98d8fdb557a920f
...
# Read the GeoJSON file with error handling and print supported formats if it fails
my_sf <- tryCatch(
  {
    read_sf(tmp_geojson)
  },
  error = function(e) {
    message("Error reading the GeoJSON file. Please check if the file is corrupt, in an unsupported format, or if there are permissions issues: ", e$message)
    message("Supported formats are: ")
    print(st_drivers())
    stop("Failed to read the GeoJSON file.")
  }
)

# Proceed with the rest of your code as before
# Ensure state names in my_sf are uppercase to match the policePerPop dataset
my_sf$name <- toupper(my_sf$name)

# Remove Alaska and Hawaii from the policePerPop dataset
policePerPop <- data %>%
  filter(!(state_name %in% c("ALASKA", "HAWAII")))

# Join the GeoJSON spatial data with the policePerPop dataset
choropleth_data <- my_sf %>%
  left_join(policePerPop, by = c("name" = "state_name"))
...
p <- ggplot(choropleth_data) +
  ...
  scale_fill_gradient(
    ...
    na.value = "grey50",
    name = "Amount of Police Officers per 100k",
    guide = guide_legend(
      keyheight = unit(3, units = "mm"),
      keywidth = unit(12, units = "mm"),
      label.position = "bottom",
  ...
  labs(
    title = "Choropleth Map of Police Officers per 100k by State",
    subtitle = "Number of police officers per 100k based on provided data",
...
ggplot(data_train_augmented, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Residuals Plot", x = "violentPerPop", y = "Residuos") +
  theme_minimal()
```
