...
map_data <- data %>%
  ...
  mutate(
    violentPerPop_n = violentPerPop / (max(violentPerPop)),
    policePerPop_n = policePerPop / (max(policePerPop))
    ) %>%
  select(State, communityname, violentPerPop, policePerPop, latitude, longitude, violentPerPop_n, policePerPop_n)
...
data_map <- leaflet(map_data) %>%
  ...
  addCircleMarkers(
    ...
    radius = ~pmax(violentPerPop_n * 15, 3),
    color = "red",
    stroke = FALSE,
    fillOpacity = 0.5,
  ...
  addCircleMarkers(
    lng = ~longitude, lat = ~latitude,
    radius = ~pmax(policePerPop_n * 15, 3),
    color = "blue",
    stroke = FALSE,
    fillOpacity = 0.5,
    popup = ~paste("Community: ", communityname, "<br>",
