...
ggplot(data_train_augmented, aes(x = .pred, y = .resid)) +
  ...
  labs(title = "Residuals Plot", x = "violentPerPop", y = "Residuals") +
  theme_minimal()
```
...
# Ensure state names in my_sf are uppercase to match the dataset
my_sf$name <- toupper(my_sf$name)

# Join the GeoJSON spatial data with the dataset
choropleth_data <- my_sf %>%
  mutate(value = violentPerPop$value[match(name, violentPerPop$state_name)])
...
p <- ggplot(choropleth_data) +
  ...
  scale_fill_gradient(
    ...
    na.value = "grey50",
    name = "Violent Crimes Commited per 100k",
    guide = guide_legend(
  ...
  labs(
    title = "Map of USA Showing Violent Crimes Commited per 100k",
