...
family_statistics_long <- family_statistics %>%
  pivot_longer(
    cols = c(Income_Levels, Two_Parent_Families, Working_Moms),
    names_to = "Category",
    values_to = "Level"
...
ggplot(family_statistics_long, 
  ...
  labs(
    title = "Boxplot of Violent Crimes per Population by Socioeconomic Factors",
    x = "Socioeconomic Levels",
    y = "Violent Crimes Per Population"
  ) +
  theme_minimal() +
  theme_bw() #Box plot showing the relationship between familial status and violent crimes
...
ggplot(data = family_statistics_long, 
  ...
  labs(
    ...
    subtitle = "Grouped by Income Levels, Number of Two Parent Families and Number of Working moms "
  ) +
  facet_wrap(~ Category,
             ncol = 3) # Histogram showing the relationship between family status and violent crimes
...
data <- read.csv("data/crime/communitiesUnnormalized.txt", header = FALSE, stringsAsFactors = FALSE, na.strings = "?")

# Load attribute names

data_names <- read_lines("data/crime/names.names")
...
map_data <- data %>%
  ...
  filter(!is.na(latitude) & !is.na(longitude) & !is.na(policePerPop) & !is.na(violentPerPop)) %>%

  select(State, communityname, violentPerPop, policePerPop, latitude, longitude)
...
data_map <- leaflet(map_data) %>%
  ...
  addCircleMarkers(
    ...
    radius = ~pmax(violentPerPop * 10, 3),  # Ensure minimum radius for visibility
    ...
    popup = ~paste("Community: ", communityname, "<br>",
                   ...
                   "Violent Crimes per Pop: ", violentPerPop)
  ...
  addCircleMarkers(
    ...
    radius = ~pmax(policePerPop * 10, 3),  # Ensure minimum radius for visibility
